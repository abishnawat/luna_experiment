...
function getPage(){
		...
		type:'post',
		cache: false,
		data: 'hd=on&position='+position+'&limit='+limit+'&scroll=true&'+'buster='+new Date().getTime()+'&'+da,
		dataType:"json",
		...
		$(document).on('click', '.play_area',function(e){
			...
			if(cur_sec!=prev_sec&&prev_sec!=0){
				...
				$('.bottom_section_area_'+cur_sec).slideDown(500,function(){
					...
					$('.video_messages').html('<img src="explore_page_imgs/balls4.gif" />Loading media, please wait...');
					prev_sec=cur_sec;
					});
				}
				else{
					...
					if(history.pushState){
						...
						History.pushState({},null,'explore_hd.php?'+elem.link);
						}
					else{
						window.location.href = 'explore_hd.php?'+elem.link;
					}
	...
	$('.search_button').click(function(e){
		 $('.suggestion_area').hide();
					...
					if($.trim(searchbox.val())!=""){
						 if(history.pushState){
							History.replaceState({'position': position,'scrollpos' : $(window).scrollTop(), 'content' : content},null,'');
							History.pushState({},null,'explore_hd.php?search='+searchbox.val());
							//$(document).focus();
							$('.search_box').val('').hide();
							$('.search_box_area').css({'width':'20px'});
							$('.search_and_suggestion').hide();
							...
							$('.suggestion_area').hide();
							  ...
							  window.location.href = 'explore_hd.php?search='+searchbox.val();
							  }	
					}
		...
		window.setTimeout(function(){
			if($.trim(searchbox.val())!=""){
				...
				if(e.keyCode==13){
					...
					$.ajax({
						url: "LunaExperiment/movie_data.php",
						data:"hd=on&scroll=true&limit=5&search="+searchbox.val(),
						type:"post",
						dataType:"json",
						cache:false,
						success: function(data){
	...
	$(document).on('click','.suggestion_individual_area',function(e) {
		...
		if(history.pushState){
							...
							History.replaceState({'position': position,'scrollpos' : $(window).scrollTop(), 'content' : content},null,'');
							history_lock=false;
							History.pushState({},null,'explore_hd.php?search='+name);
							//$(document).focus();
							$('.search_box').val('');
							...
							$('.search_and_suggestion').hide();
							  ...
							  window.location.href = 'explore_hd.php?search='+name;
							  }	
		}
    });
